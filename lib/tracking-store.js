...
async function deleteSpamInquiry(id, deps) {
  const uuid = clean(id, 40);
  if (!/^[0-9a-f-]{36}$/i.test(uuid)) throw new Error("invalid_inquiry_id");
  const result = await supabaseRequest(
    `${TABLE}?id=eq.${encodeURIComponent(uuid)}&status=eq.spam`,
    {
      method: "DELETE",
      headers: { Prefer: "return=representation" }
    },
  ...
  if (!Array.isArray(result) || result.length !== 1) {
    const error = new Error("inquiry_not_deletable");
    error.code = "INQUIRY_NOT_DELETABLE";
    throw error;
  }
  return result[0];
}
